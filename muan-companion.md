...
| `SLIDEV_MUAN_COMPANION_PRESENTATIONS_DIR` | Root directory scanned for presentable decks. Each immediate subdirectory with a `slides.md` and the companion addon configured becomes one entry on `/home`. Unset means the list is empty and server-launched decks are impossible — a complete no-op.                           | _unset_                                        | No. Required to use Flow A (server-launched decks) at all.                                                                                                                                                                                 |
| `SLIDEV_MUAN_COMPANION_PUBLIC_URL`        | Where _this_ server is reachable **from a participant's browser**. Injected into every server-launched deck as `VITE_SLIDEV_MUAN_COMPANION_SERVER_URL`, and its scheme+host is the origin a launched deck's own URL is built from (with the deck's port).                          | `http://localhost:$PORT`                       | No for a local trial; **yes, set it** for any real Flow-A workshop — the default is only correct for participants on the operator's own machine.                                                                                           |
| `SLIDEV_MUAN_COMPANION_MAX_SPAWNED_DECKS` | Cap on how many decks this server will have running at once. Past it, Present returns a 503 with a clear error instead of degrading silently.                                                                                                                                      | `4`                                            | No.                                                                                                                                                                                                                                        |
| `SLIDEV_MUAN_COMPANION_SPAWN_REMOTE`      | `true` starts server-launched decks with `--remote=`, binding them to every interface instead of `localhost`. **Needed for participants on other devices to reach a launched deck**, and off by default because opening a port on every interface is a deliberate decision.        | _unset_ (off)                                  | No, but a real Flow-A workshop needs it.                                                                                                                                                                                                   |
| `SLIDEV_MUAN_COMPANION_SLIDEV_BIN`        | Explicit path to the Slidev CLI for server-launched decks. Normally unnecessary: the server resolves the deck's own `node_modules/.bin/slidev`, walking up from the deck folder, then falls back to `PATH`.                                                                        | _unset_                                        | No.                                                                                                                                                                                                                                        |
