...
the workshop-tracker sync server (plan 027, PRD §8).
...
async function onCopy() {
  if (props.command) {
    try {
      ...
      console.error('[workshop-tracker] clipboard write failed', error)
    }
  }
  apply('click-copy')
...
<template>
  <div class="workshop-tracker-step-command">
    <code v-if="command" class="workshop-tracker-step-command-code">{{ command }}</code>
    <div class="workshop-tracker-step-command-actions">
      <button
        v-if="command"
        type="button"
        class="workshop-tracker-step-command-button"
      ...
      <button
        type="button"
        class="workshop-tracker-step-command-button workshop-tracker-step-command-button-done"
...
.workshop-tracker-step-command-code {
  flex: 1 1 auto;
  min-width: 0;
  overflow-x: auto;
  white-space: pre;
}
.workshop-tracker-step-command-actions {
  display: flex;
  gap: 0.5em;
  flex: 0 0 auto;
}
.workshop-tracker-step-command-button {
...
.workshop-tracker-step-command-button:disabled {
  cursor: default;
  opacity: 0.7;
}
.workshop-tracker-step-command-button-done {
