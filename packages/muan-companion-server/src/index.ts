...
const roomCode = process.env.MUAN_COMPANION_ROOM_CODE
const presenterCode = process.env.MUAN_COMPANION_PRESENTER_CODE
...
if (!roomCode || !presenterCode) {
  console.warn(
    '[muan-companion-server] MUAN_COMPANION_ROOM_CODE and/or MUAN_COMPANION_PRESENTER_CODE '
...
const { httpServer } = createMuanCompanionServer({
  origin: process.env.MUAN_COMPANION_ORIGIN ?? '*',
  roomCode,
  presenterCode,
})
