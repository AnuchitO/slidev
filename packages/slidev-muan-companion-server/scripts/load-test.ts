/**
 * Load test for `slidev-muan-companion-server` (plan 030 Step 2 / PRD §12,
 * §15). Spins up the server in-process (via `createMuanCompanionServer`,
 ...
 *
 * Usage:
 *   pnpm --filter slidev-muan-companion-server load-test [-- --participants=100]
 *
...
import type { MuanCompanionServer } from '../src/server'
...
import { createMuanCompanionServer } from '../src/server'
...
async function main() {
  ...
  const server: MuanCompanionServer = createMuanCompanionServer({ roomCode, presenterCode })
  ...
  console.log(`\n=== slidev-muan-companion-server load test: ${participantSockets.length} participants ===`)
...
Generated by \`pnpm --filter slidev-muan-companion-server load-test\` — re-run this
